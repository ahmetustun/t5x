...
from t5x import models, partitioning
...
def run_actions(mode: trainer_lib.ActionMode, actions: trainer_lib.ActionMapType,
...
def train(
    *,
    ...
    verify_matching_vocabs_fn: Optional[Callable[[utils.DatasetConfig, models.BaseTransformerModel],
  ...
  """
    ...
    if use_orbax:
        ...
        if (checkpoint_cfg.save and isinstance(checkpoint_cfg.save.checkpointer_cls, pw_checkpoints.Checkpointer)) or (
                checkpoint_cfg.restore and
                isinstance(checkpoint_cfg.restore.checkpointer_cls, pw_checkpoints.Checkpointer)):
            raise ValueError('Requested use_orbax with Pathways checkpointing, which is currently'
                             ' unsupported.')
    ...
    checkpoint_steps = (checkpoint_cfg.save.checkpoint_steps if checkpoint_cfg.save else [])

    if eval_period or checkpoint_period or gc_period:
        steps_per_epoch = min(eval_period or np.inf, checkpoint_period or np.inf, gc_period or np.inf)
    else:
        steps_per_epoch = total_steps
    stats_period = stats_period or steps_per_epoch
    if (eval_period and eval_period % steps_per_epoch or checkpoint_period and checkpoint_period % steps_per_epoch or
            gc_period and gc_period % steps_per_epoch):
        raise ValueError(f'Checkpoint period ({checkpoint_period}), eval '
                         f'period ({eval_period}), and GC period ({gc_period}) must all be '
                         'multiples of each other.')

    if use_hardware_rng or random_seed is None:
        logging.info('Using fast RngBitGenerator PRNG for initialization and dropout.')
        ...
        if random_seed is None:
            ...
            logging.warning('When using hardware RNG with a fixed seed, repeatability is only '
        ...
        logging.info('Using seed for initialization and dropout RNG: %d', random_seed)
        rng = random.PRNGKey(random_seed)
    ...
    if (train_dataset_cfg.seed and not (checkpoint_cfg.save and checkpoint_cfg.save.save_dataset)):
        logging.warning('Providing a random seed for the train dataset with '
    ...
    train_iter = get_dataset_fn(train_dataset_cfg, ds_shard_id, num_ds_shards, model.FEATURE_CONVERTER_CLS)
    train_iter = utils.prepare_train_iter(train_iter,
                                          checkpoint_cfg=checkpoint_cfg,
                                          partitioner=partitioner,
                                          data_layout=data_layout)

    input_shapes = jax.tree_map(lambda x: (data_layout.batch_size, *x.shape[1:]), train_iter.element_spec)
    ...
    if train_eval_dataset_cfg:
        ...
        if not train_eval_datasets:
            logging.warning('No train_eval datasets loaded from config `train_eval_dataset_cfg`: '
    ...
    if checkpoint_cfg.restore is not None:
        state_transforms_for_restore = [
            functools.partial(fn, is_resuming=True) for fn in checkpoint_cfg.restore.state_transformation_fns
        ]
    else:
        state_transforms_for_restore = []
    restore_cfgs = [
        utils.RestoreCheckpointConfig(
            ...
            checkpointer_cls=checkpoint_cfg.save.checkpointer_cls if checkpoint_cfg.save else checkpoints.Checkpointer,
            # Restore dataset state if it is being saved.
            restore_dataset=(checkpoint_cfg.save and checkpoint_cfg.save.save_dataset),
    ...
    if checkpoint_cfg.restore:
        if checkpoint_cfg.restore.mode == 'all':
            raise ValueError("Restore checkpoint mode 'all' is not supported in training.")
        ...
        if isinstance(checkpoint_cfg.restore.path, str):
            ...
            raise ValueError('Restore checkpoint config may only have a single path in training.')

    init_or_restore_tick = time.time()
    train_state_initializer = train_state_initializer_cls(optimizer_def=model.optimizer_def,
    ...
    # May be None, empty
    valid_restore_cfg, restore_paths = (utils.get_first_valid_restore_config_and_paths(restore_cfgs))
    ...
    if valid_restore_cfg or checkpoint_period or checkpoint_steps:
        train_state, checkpoint_manager = (utils.create_checkpoint_manager_and_restore(
    ...
    logging.info('Initialize/restore complete (%.2f seconds).', init_or_restore_secs)
    ...
    train_metrics.write_scalar('timing/init_or_restore_seconds', init_or_restore_secs, host_step)
    ...
    if infer_eval_dataset_cfg is not None:
        evaluator = eval_lib.InferenceEvaluator(infer_eval_dataset_cfg=infer_eval_dataset_cfg,
    ...
    if concurrent_metrics and actions.get(trainer_lib.ActionMode.INFER_EVAL, None) is not None:
        logging.warning('Actions for INFER_EVAL will not be triggered when async '
                        'metrics computation is enabled')
    if concurrent_metrics and actions.get(trainer_lib.ActionMode.TRAIN, None) is not None:
    ...
    def _run_training_eval(first_run: bool = False):
        if first_run:
            ...
            trainer.compile_eval({  # pytype: disable=wrong-arg-types  # jax-ndarray
                task: utils.get_zeros_batch_like_dataset(ds) for task, ds in train_eval_datasets.items()
            })
        logging.info('Computing training evaluation metrics.')
        eval_batch_iters = {task: ds.as_numpy_iterator() for task, ds in train_eval_datasets.items()}
    ...
    def _run_inference_eval():
        ...
        if not concurrent_metrics:
            ...
            trainer.stop_training = run_actions(trainer_lib.ActionMode.INFER_EVAL, actions, trainer.train_state,
                                                all_metrics_done)
        train_metrics.write_scalar('timing/evaluate_seconds', time.time() - evaluate_tick, host_step)
    ...
    if checkpoint_period:
        ...
        # exists.
        if not use_orbax or (use_orbax and
                             utils.get_local_data(trainer.train_state.step) not in checkpoint_manager.all_steps()):
    ...
    if total_steps < first_step:
        raise ValueError(f'Unexpected total_steps ({total_steps}) < checkpoint step '
                         f' ({first_step}).')
    ...
    num_epochs = first_epoch + math.ceil((total_steps - first_step) / steps_per_epoch)
    logging.info('Training with artificial "epochs" of %d steps.', steps_per_epoch)
    ...
    def _as_gda(spec):
        ...
        return jax.make_array_from_callback(
            dummy.shape,
            jax.sharding.NamedSharding(partitioner.mesh, partitioner.data_partition_spec),
    ...
    train_metrics.write_scalar('timing/train_iter_warmup', train_iter_warmup_tock - train_iter_warmup_tick, host_step)

    jax.monitoring.record_event_duration_secs('/jax/t5x/train/time_before_first_step_secs', time.time() - _IMPORT_TIME)
    ...
    for epoch in range(first_epoch, num_epochs):
        ...
        try:
            ...
            while host_step < epoch_end_step:
                ...
                # save old steps
                checkpoint_steps_index = utils.find_first_checkpoint_step(checkpoint_steps_index, checkpoint_steps,
                                                                          first_step, host_step)
                ...
                train_summary = trainer.train(train_iter, inner_num_steps, start_step=host_step)
                if not concurrent_metrics:
                ...
                if (checkpoint_steps and checkpoint_steps_index < len(checkpoint_steps) - 1):
        ...
        if checkpoint_period and (final_epoch or step_offset % checkpoint_period == 0):
        ...
        is_eval_epoch = eval_period and (final_epoch or step_offset % eval_period == 0)
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string(
        'tfds_data_dir', None, 'If set, this directory will be used to store datasets prepared by '
        'TensorFlow Datasets that are not available in the public TFDS GCS '
        'bucket. Note that this flag overrides the `tfds_data_dir` attribute of '
        'all `Task`s.')

    flags.DEFINE_list('seqio_additional_cache_dirs', [],
                      'Directories to search for cached Tasks in addition to defaults.')

    flags.DEFINE_boolean('multiprocess_gpu',
                         False,
    ...
    def _main(argv: Sequence[str]):
        ...
        if FLAGS.multiprocess_gpu:
            logging.info(
                ...
                '  coordinator_address: %s\n  process_count: %s\n  process_index: %s', FLAGS.coordinator_address,
                FLAGS.process_count, FLAGS.process_index)
